...
export default function Models() {
  ...
  for (let i = 0; i < data.data.length; i++) {
    models.push({display_name: data.data[i].attributes.display_name, profile_pic: data.data[i].attributes.profile_pic.data.attributes.url});
  }
    ...
    {models.map((item) => {
      ...
      <div class="flex flex-wrap p">
            <img
              ...
              item.profile_pic} />
              <h2 class="mt-4" style={{fontSize: '30px', fontWeight: 'bold', textAlign: 'center', width: '100%'}}>{item.display_name}</h2>
        </div>
        </a>)
    })}
    </div>
